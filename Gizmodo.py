...
def lambda_handler(event, context):
    gizmodo_feed_url = r"https://gizmodo.com/rss"
    feed = feedparser.parse(gizmodo_feed_url).entries #get the entries in the feed
    ...
    for entry in feed: #iterate over the feed
        ...
        gizmodo_feed.append(template_dict) #append the filled out dictionary to the list of dictionaries
    lambda_output = [dict(t) for t in {tuple(gizmodo_feed_dict.items()) for gizmodo_feed_dict in gizmodo_feed}] #remove all duplicates
    ...
    s3.Bucket(bucket_name).put_object(Key=f"Gizmodo/feed_{formatted_date}.json", Body=as_json)
